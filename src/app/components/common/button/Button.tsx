...
function Button({
  ...
  const baseStyles =
    'items-center font-semibold text-text-default justify-center rounded-xl';
  ...
  const variantStyles = {
    ...
    inverse:
      size === 'small'
        ? 'bg-none border-brand-primary border-[1px] text-brand-primary hover:border-interaction-hover hover:text-interaction-hover active:border-interaction-pressed active:text-interaction-pressed'
    ...
    danger: 'bg-point-red hover:bg-red-600',
  };
  ...
  const disabledStyles = {
    ...
    inverse:
      size === 'small'
        ? 'bg-none border-[1px] border-interaction-inactive text-interaction-inactive cursor-not-allowed'
  ...
  const sizeStyles = {
    small: 'px-3 py-2 text-md w-[74px] h-[32px]',
